 ...
 * @returns void
 * Apaga a varList da stack, e os valores associados a cada variável
*/
 ...
 * @param stack que se pretende apagar
 * @returns void
 * Apaga a stack, incluindo o valor dos nodes individualmente. Apaga a varList da stack.
*/
...
/**
 * Copia o NODE src para dest.
 * **Assume que dest->value tinha informação e faz free**
...
void printBlock (void *value) {
    BLOCKPTR block = value;
    if (*block->string) printf("{ %s }", (char *) block->string);
    else printf("{ }");
}
...
/**
 * Usada pela debugPrintStack para tornar debug de arrays mais legível
...
/**
 * Faz print da stack, indicando tipos, valores, inicio e fim de arrays, varList, etc.
...
/**
 * Semelhante a debugPrintStack(), mas não indica a varList
 * @param stack para dar print
 * @param printDist dispatch table de funções de print
 * @param depth para tornar legíveis nested arrays
 * @returns void
...
/**
 * Faz debug da stack, chamando simpleDebugPrint()
...
/**
 * Faz debug da stack, chamando debugPrintStack()
 * @param stack para a qual fazer debug
 * @returns void
*/
...
/**
 * Semelhante a push(), mas o valor é um void* e não long double
 * @param stack para a qual dar push
...
/**
 * Faz pop do elemento no topo da stack. Decrementa sp e chama deleteNode()
...
STACK getSubstrings (STACK stack, NODE topNode, NODE sndNode) {
    if (topNode->type == CHAR) { //solução manhosa
    ...
    *temp;
    ...
    if (length) { //se for 0 temos de separar string para array de chars
        ...
        for (; *str; str++) {
            ...
            newStr[0] = str[0];
            newStr[1] = '\0';
            generalPush(newstack, newStr, STRING);
        }
    }
    return newstack;
...
//multiplica numeros, arrays e strings
...
//faz map dos elementos do array em sndNode aplicado o bloco em topNode
...
/**
 * Passa topo da stack para int, chamando nodeToInt() (para truncar resultado)
 * @param stack onde executar operação
 * @returns void
*/
...
//dado um array com os valores (de getArray values), cria uma lista tempOrganizer ordenada
...
//aplica o bloco para criar um array com os valores que sao usados para ordenação
//retorna uma lista tempOrganizer já ordenada
...
void top2Greater (STACK stack) {
    ...
    if (topNode->type == STRING) {
        if (strcmp(topNode->value, sndNode->value) > 0) {
...
/**
 * compara os dois valores no topo da stack, segundo a operação "or", parando se o primeiro for logo verdade (isto é, tem shortcut), manipulando a stack
 * @param stack 
 * @return não retorna nada
*/
...
/**
 * compara os dois valores no topo da stack, segundo a operação "and", parando se o primeiro for logo falso (isto é, tem shortcut), manipulando a stack
 * @param stack 
 * @return não retorna nada
*/
...
void executew (STACK stack) {
    ...
    BLOCKPTR block = topNode->value;
    topNode->value = NULL;
    stack->sp--; // ignorar bloco
    topNode->type = 0;
    int i;
    do {
        mainHandler(block->string,block->dist, block->handle, stack);
        i = getLogicalValue(stack->array + stack->sp);
        pop(stack); //remover condição
    } while (i);
    //apagar bloco
    free(block->string);
    free(block);
...
void toString (STACK stack) {
    ...
    } else if (topNode->type == CHAR) {
        str = malloc(sizeof(char)*2);
        str[0] = (char)*(long double *)topNode->value;
        str[1] = '\0';
    } else {
